...
def get_errors_plot(userId: str = Query(..., description="The user ID to filter data")):
    ...
    fig = px.bar(error_counts, x="Error Type", y="Count", title=f"Error Distribution by Type for User {userId}")
...
def code_metrics_radar_chart(userId: str = Query(..., description="The user ID to filter data")):
    ...
    fig.update_layout(
        ...
        title=f'Code Quality Metrics (Radar Chart) for User {userId}',
